PREFIX = '/usr'
DESTDIR = ''
TEMPDIR := $(shell mktemp -u --suffix .systemd-zram)
...
clean:
	rm -rf *.xz *.gz *.tgz *.deb *.rpm ChangeLog /tmp/tmp.*.systemd-zram debian/changelog debian/README debian/files debian/systemd-zram debian/debhelper-build-stamp debian/systemd-zram*
